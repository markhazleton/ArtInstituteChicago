...
public class ArtworkFilterController : Controller
{
    ...
    public async Task<IActionResult> CompareStyles(ArtStyle[]? styles, int limit = 6)
    {
        try
        {
            ...
            var comparisonResults = new Dictionary<ArtStyle, IEnumerable<ArtInstituteChicago.Client.Models.Collections.ArtWork>>();
            ...
            foreach (var style in styles.Take(4)) // Limit to 4 styles for display purposes
            {
                ...
                comparisonResults[style] = response?.Data ?? new List<ArtInstituteChicago.Client.Models.Collections.ArtWork>();
            }
